# Power BI Dashboard For French Patisserie

This project consists of a Power BI dashboard created in collaboration with Nokia during my university studies. 

---

## Dashboard Preview

![Dashboard Preview](link_to_image_here)

---

## Context and Objective

The dashboard was developed for [department/project/team] to visualize and analyze [type of data, KPIs, trends]. The main goal was to [short goal: e.g., improve decision-making through interactive visualizations].

---

## Key Features

- Interactive visualization of key metrics
- Filtering by [relevant criteria]
- Trend analysis over time
- Export reports to PDF

---


## How to Use

1. Open `Dashboard.pbix` in Power BI Desktop.
2. Interact with filters and visualizations.

---

## Technologies and Tools

- Power BI Desktop
- DAX (Data Analysis Expressions) for calculated measures 
- Data sourced from Kaggle.com and processed in phpMyAdmin

---

## Collaboration

Project developed in collaboration with the Nokia team during my university studies at West University of Timișoara.

---

## License / Disclaimer

This project is for educational purposes only and does not contain any confidential or proprietary data.
